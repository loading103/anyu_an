...
public class LiveGiftAnimPresenter {
    ...
    private CommonCallback<String> mDownloadGifCallback2;
    private int mDp10;
    private int mDp500;
    ...
    public LiveGiftAnimPresenter(Context context, View v, GifImageView gifImageView, SVGAImageView svgaImageView, ViewGroup liveGiftPrizePoolContainer) {
        ...
        mDownloadGifCallback2 = new CommonCallback<String>() {
            @Override
            public void callback(String assets) {
                if (assets != null) {
                    playHaoHuaGift(null,assets);
                } else {
                    mShowGif = false;
                }
            }
    ...
    private void showGifGift(LiveReceiveGiftBean bean) {
        ...
        if (mShowGif) {
            ...
            try {
                mContext.getAssets().open(s[s.length-1]);
                L.e("gif礼物"+"----------->存在");
                mDownloadGifCallback2.callback(s[s.length-1]);
            } catch (IOException e) {
                e.printStackTrace();
                L.e("gif礼物"+"----------->不存在");
...
//        params.height = (int) (mSVGAImageView.getWidth() * h / w);
     ...
     * 播放豪华礼物
     */
    private void playHaoHuaGift(File file,String assets) {
     ...
     * 播放gif
     */
    private void playGift(File file,String assets) {
        try {
            if(file==null){
                mGifDrawable = new GifDrawable(mContext.getAssets().openFd(assets));
            }else {
     ...
     * 播放svga
     */
    private void decodeSvga(File file,String assets) {
        if (mSVGAParser == null) {
        ...
        try {
            if(file!=null){
                ...
                mSVGAParser.decodeFromAssets(assets,mParseCompletionCallback);
            }
