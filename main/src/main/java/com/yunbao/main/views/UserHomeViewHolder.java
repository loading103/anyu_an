...
import com.yunbao.main.activity.ChoosePhotoActivity;
...
public class UserHomeViewHolder extends AbsLivePageViewHolder implements AppBarLayout.OnOffsetChangedListener, LiveShareDialogFragment.ActionListener {
    ...
    public void init() {
        ...
        mBtnEdit = (ImageView) findViewById(R.id.btn_edit);
        if (mSelf) {
        ...
        MainHttpUtil.getUserHome(mToUid, new HttpCallback() {
            ...
            public void onSuccess(int code, String msg, String[] info) {
                if (code == 0 && info.length > 0) {
                    ...
                    userBean = JSON.toJavaObject(obj, SearchUserBean.class);
                    mSearchUserBean = userBean;
                    String avatar = userBean.getAvatar();
                    ImgLoader.displayBlur(CommonAppContext.sInstance,avatar, mAvatarBg);
                    ImgLoader.displayAvatar(CommonAppContext.sInstance,avatar, mAvatar);
                    ...
                    ImgLoader.display(CommonAppContext.sInstance,levelAnchor.getThumb(), mLevelAnchor);
                    LevelBean level = appConfig.getLevel(userBean.getLevel());
                    ImgLoader.display(CommonAppContext.sInstance,level.getThumb(), mLevel);
                    if(mToUid.equals(CommonAppConfig.getInstance().getUid())){
                        mID.setText(userBean.getZhangHao());
                    }else {
                    ...
                    if(mUserHomeSharePresenter!=null&&mToUid!=null&&userBean!=null){
    ...
    private void showContribute(String conJson) {
        ...
        for (int i = 0, size = list.size(); i < size; i++) {
            ...
            ImgLoader.display(CommonAppContext.sInstance,list.get(i).getAvatar(), imageView);
    ...
    public void updataHeader(String url) {
        ImgLoader.displayBlur(CommonAppContext.sInstance,url, mAvatarBg);
        ImgLoader.displayAvatar(CommonAppContext.sInstance,url, mAvatar);
    }
}
