...
import com.yunbao.common.custom.CommonRefreshView3;
...
public class MainHomeVideoViewHolder extends AbsMainHomeChildViewHolder implements OnItemClickListener<VideoBean> {
    ...
    private CommonRefreshView3 mRefreshView;
    ...
    public void init() {
        ...
        tvFinish = (TextView) findViewById(R.id.tv_right);
        ...
        String name = SpUtil.getInstance().getStringValue(SpUtil.VIDEO_NAME);
        if(!TextUtils.isEmpty(name)){
            titleView.setText(name);
        }
        tvFinish.setVisibility(View.GONE);
        tvFinish.setTextColor(mContext.getResources().getColor(R.color.white));
        tvFinish.setText("编辑");
        tvFinish.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(tvFinish.getText().toString().equals("编辑")){
                    tvFinish.setText("取消");
                    mainDownLoadViewHolder.edit(true,tvFinish);
                }else {
                    tvFinish.setText("编辑");
                    mainDownLoadViewHolder.edit(false,tvFinish);
                }
            }
        });


        mRefreshView = (CommonRefreshView3) findViewById(R.id.refreshView);
        ...
        mRefreshView.setDataHelper(new CommonRefreshView3.DataHelper<VideoBean>() {
            @Override
        ...
        if (configBean != null) {
            ...
            if (list != null && list.size() > 0) {
                ...
                if(!list.get(1).getName().equals("推荐")){
                    list.add(0,bean1);
                    list.add(1,bean3);
                }
...
//                }
                if (position==0) {//下载
                    ...
                    if(mainDownLoadViewHolder!=null){
                        mainDownLoadViewHolder.refresh();
                    }
                    return;
                }
                tvFinish.setVisibility(View.GONE);
...
//                    showClassListDialog();
    ...
    public void onVideoScrollPageEvent(ShowRedEvent e) {
        if(topAdapter!=null){
            topAdapter.setShowred(e.isShowred());
            topAdapter.notifyDataSetChanged();
        }
    }
    @Subscribe(threadMode = ThreadMode.MAIN)
