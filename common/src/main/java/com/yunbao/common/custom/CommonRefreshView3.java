...
public class CommonRefreshView3 extends FrameLayout implements View.OnClickListener {
    ...
    public CommonRefreshView3(Context context) {
        this(context, null);
    }

    public CommonRefreshView3(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CommonRefreshView3(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onFinishInflate() {
        ...
        mSmartRefreshLayout.setOnRefreshListener(new OnRefreshListener() {
            ...
            public void onRefresh(@NonNull RefreshLayout refreshLayout) {
                //用于首页顶部数据下拉需要刷新
                SpUtil.getInstance().setBooleanValue(SpUtil.REFRESH_TOP,true);
                refresh();
            }
        });
    ...
    private HttpCallback mRefreshCallback = new HttpCallback() {
        ...
        public void onCacheSuccess(Response<JsonBean> response) {
            ...
            if (response != null) {
                List list = mDataHelper.processData(response.body().getData().getInfo());
                if (list == null) {
                    return;
                }
                mDataCount = list.size();
...
//                    mSmartRefreshLayout.finishLoadMoreWithNoMoreData();
    ...
    public ClassicsFooter getFooter() {
        return mFooter;
    }
